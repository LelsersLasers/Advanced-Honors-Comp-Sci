...
def detect_largest_faces(image, conf, count):
    faces = detect_faces_dnn(image, conf)
    faces.sort(key=Face.calc_area, reverse=True)
    
    if len(faces) < count:
        raise Exception(f'Not enough faces detected. {len(faces)} detected, {count} requested.')
...
ap.add_argument("-n", "--number", required = True, help = "Number of faces to detect")
...
faces = detect_largest_faces(original_image, conf, number)
